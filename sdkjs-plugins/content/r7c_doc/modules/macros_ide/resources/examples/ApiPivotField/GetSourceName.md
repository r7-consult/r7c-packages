/**
 * @file GetSourceName_macroR7.js
 * @brief R7 Office JavaScript Macro - ApiPivotField.GetSourceName
 * @author R7-Consult
 * @version 1.0.0
 * @date July 15, 2025
 *
 * @description
 * This macro demonstrates how to get the source name of a pivot field.
 * It creates a pivot table, adds fields, renames a pivot field, and then displays both the new name and the source name.
 *
 * @description (Russian)
 * Этот макрос демонстрирует, как получить исходное имя поля сводной таблицы.
 * Он создает сводную таблицу, добавляет поля, переименовывает поле сводной таблицы, а затем отображает как новое имя, так и исходное имя.
 *
 * @returns {void}
 *
