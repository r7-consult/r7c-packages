...
(function() {
    ...
    if (Api.GetActiveSheet) {
        ...
        for (var i = 0; i < count; i++) {
            ...
            var makeColor = Api.CreateColorFromRGB || Api.CreateRGBColor;
            
            // Set different colors based on parameter
            switch (color.toLowerCase()) {
                case 'red':
                    oRange.SetFillColor(makeColor.call(Api, 255, 0, 0));
                    break;
                case 'blue':
                    oRange.SetFillColor(makeColor.call(Api, 0, 0, 255));
                    break;
                case 'green':
                    oRange.SetFillColor(makeColor.call(Api, 0, 255, 0));
                    break;
                case 'yellow':
                    oRange.SetFillColor(makeColor.call(Api, 255, 255, 0));
                    break;
                default:
                    oRange.SetFillColor(makeColor.call(Api, 128, 128, 128)); // Gray
            }
